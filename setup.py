...
def configuration(top_path=""):
    ...
    fexact = Ext(
        name="FisherExact.statlib.fexact",
        sources=[os.path.join(top_path, x) for x in fexact_sources],
    )
    asa159 = Ext(
        name="FisherExact.statlib.asa159",
        sources=[os.path.join(top_path, "FisherExact/statlib", "asa159.f90")],
    )
    asa205 = Ext(
        name="FisherExact.statlib.asa205",
        sources=[os.path.join(top_path, "FisherExact/statlib", "asa205.f90")],
    )
...
def setup_package():
    ...
    setup(
        ...
        download_url="https://github.com/maclandrol/FisherExact/archive/%s.tar.gz"
        % __version__,
        author=__author__,
